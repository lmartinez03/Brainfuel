...
/**
 * Design intent fonts: "Baloo 2" (chunky, for headings & numbers) and
 * "Nunito" (rounded, for body). They are NOT applied as fontFamily here so the
 * code runs with zero extra setup (the allowed libraries don't include a font
 * loader). We approximate them with heavy font weights. To match the design
 * 1:1, load these families in a real project and set `fontFamily` from here.
 */
export const fonts = {
  heading: undefined as string | undefined, // e.g. 'Baloo2_700Bold'
  body: undefined as string | undefined,     // e.g. 'Nunito_800ExtraBold'
  // weights we lean on to mimic the chunky look
  weight: {
